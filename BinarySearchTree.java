...
public class BinarySearchTree<K extends Comparable<K>, V> implements OrderedDeletelessDictionary<K,V> {
    ...
    public K findNextKey(K key){
        ...
        // Whenever we see a key larger than key save it in prev
        K next = null;
        // Search for the given tree while storing any keys larger than it until we hit a leaf
        while (cur_node != null) {
            // If current key is larger than the key move left and save to next
    ...
    public K findPrevKey(K key){
        ...
        // Whenever we see a key larger than key save it in prev
        K prev = null;
        // Search for the given tree while storing any keys smaller than it until we hit a leaf
        while (cur_node != null) {
            // If current key is larger than or equal to key move left
            if (cur_node.key.compareTo(key) >= 0) {
                cur_node = cur_node.left;
            // Else save current key to prev and move right
            } else {
                prev = cur_node.key;
                cur_node = cur_node.right;
            }
